...
TocOpen: true
faq:
  - question: "포커 토너먼트와 캐시게임의 가장 큰 차이는 무엇인가요?"
    answer: "캐시게임은 칩이 곧 돈이지만, 토너먼트는 칩이 순위와 상금으로 전환됩니다. ICM(Independent Chip Model) 개념 때문에 토너먼트에서는 칩 EV보다 달러 EV를 고려한 플레이가 필요합니다."
  - question: "토너먼트 버블에서는 어떻게 플레이해야 하나요?"
    answer: "버블 상황에서는 숏스택을 압박하고, 미디엄 스택은 리스크를 최소화하며, 칩 리더는 공격적으로 상대를 압박하는 전략이 유효합니다. ICM 압박을 이해하고 상황에 맞는 폴드도 중요합니다."
  - question: "파이널 테이블에서 중요한 전략은 무엇인가요?"
    answer: "파이널 테이블은 페이 점프가 크기 때문에 ICM이 더욱 중요합니다. 숏스택 제거를 노리고, 상금 구조를 파악해 리스크 조절이 필요하며, 헤즈업까지 간다면 공격적 전환이 필수입니다."
---
